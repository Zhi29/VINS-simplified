...
string sData_path = "/Users/zhili/Documents/C++/course2_hw_new/vio_data_simulation/bin/";
...
void PubImuData()
{
	...
	cout<<"1 PubImuData start sImu_data_files: "<<sImu_data_file<<endl;
	ifstream fsImu;
	fsImu.open(sImu_data_file.c_str());
	if (!fsImu.is_open())
	{
...
void PubImageData()
{
    ...
    if(!fsImage.is_open())
    {
        cerr<<"Failed to open time_files.txt"<<sImage_file<<endl;
        return;
    }
    ...
    while(std::getline(fsImage, sImage_line) && !sImage_line.empty())
    {
        ...
        std::string imagePath = sData_path + sImgFileName;
